...
def from_excel(xls_file=None, pattern=r"OceanScan"):
    ...
    sheets = is2cv.utilities.get_excel_sheet_names(
        xls_file, pattern=r"Cycle\s+(\d+)"
    )
...
def get_granules(df, product="ATL12", release=7, timeout=60):
    ...
    for i, row in df2.iterrows():
        ...
        if not granule.exists():
            is2tk.utilities.from_nsidc(
                row["url"], local=granule, timeout=timeout, verbose=True
            )
...
def arguments():
    ...
    parser.add_argument(
        "--release", "-r", type=int, default=7, help="ICESat-2 Data Release"
    )
    # event type
    parser.add_argument(
        "--event",
...
def main():
    ...
    if args.download:
        get_granules(
            df, product=args.product, release=args.release, timeout=args.timeout
        )
    ...
    for i, row in df.iterrows():
        ...
        granules = find_granules(
            row, product=args.product, release=args.release
        )
        ...
        outfile = (
            f"{args.product}_{args.event}_Cycle{row.cycle:02.0f}_"
