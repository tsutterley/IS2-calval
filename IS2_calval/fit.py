...
def scan_design_matrix(x, y, t, az, t0=None, nadir=False):
    ...
    G += sp.coo_matrix(
        (v * np.cos(az[r] * d2r), (r, c + 2 * nci)), shape=G.shape
    )
    # quad-phase scan offset in x
    G += sp.coo_matrix(
        (v * np.sin(az[r] * d2r), (r, c + 3 * nci)), shape=G.shape
    )

    # in-phase scan offset in y:
    G += sp.coo_matrix(
        (v * np.cos(az[r] * d2r), (r + nri, c + 4 * nci)), shape=G.shape
    )
    # quad-phase scan offset in y
    G += sp.coo_matrix(
        (v * np.sin(az[r] * d2r), (r + nri, c + 5 * nci)), shape=G.shape
    )
    ...
    Gci = sp.diags(
        [np.zeros(nrc) + 0.01, np.zeros(nrc) + -0.02, np.zeros(nrc) + 0.01],
        [0, 1, 2],
        shape=[nrc, ncc],
    )
    r, c, v = sp.find(Gci)
...
def eval_ATM_scan(
    x, y, t, az, t0, xy_nadir=None, phase_xy=None, m=None, nadir=False
):
    if m is None:
        if nadir:
...
def fit_ATM_data(D, calc_fit=False):
    ...
    t0 = np.linspace(
        np.min(t), np.max(t), np.max([20, np.ceil(n_secs / 2)]).astype("i")
    )

    good = np.flatnonzero(
        np.isfinite(D.x + D.y + t) & (np.abs(D.x + 1j * D.y) < 1.0e7)
    )
    good = good[::50]
    xy_naidr0, amp, phase_xy, t0, m = fit_ATM_scan(
        D.x[good], D.y[good], t[good], D.azimuth[good], t0=t0
    )
    x_off, y_off, x_nadir, y_nadir = calc_xt_offset(
        D.x, D.y, t, D.azimuth, t0, m=m
    )
