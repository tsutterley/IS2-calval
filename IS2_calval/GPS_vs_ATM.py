...
def blockmedian_for_gps(GPS, delta):
    ...
    delta_lon = np.mod(GPS.longitude - lon0 + 180.0, 360.0) - 180
    # project the GPS latitude and longitude into northing and easting
    EN = (
        Re
        * np.c_[delta_lon * np.cos(d2r * lat0), (GPS.latitude - lat0)]
        * np.pi
        / 180.0
    )
    z = GPS.z.astype(np.float64)
    xm, ym, zm, ind = pt_blockmedian(
        EN[:, 0], EN[:, 1], z, delta, return_index=True
...
def compare_gps_with_qfit(GPSsub, Qdata, out_template):
    ...
    sub_10m = np.logical_and(
        np.abs(xy_local[:, 1]) < 10, np.abs(xy_local[:, 0]) < 10
    )
    if np.sum(sub_10m) > 5:
...
def main():
    ...
    print(
        "working on GPS file {0}, ATM directory {1}".format(GPS_file, ATM_dir)
    ...
    GPS = (
        pc.data().from_h5(GPS_file, field_dict=GPS_field_dict).get_xy(SRS_proj4)
    )
