...
def compare_seg_with_gps(D6i, GPS, out_template):
    ...
    EN = (
        Re
        * np.c_[
            (np.mod(GPS.longitude - lon0 + 180.0, 360.0) - 180)
            * np.cos(d2r * lat0),
            (GPS.latitude - lat0),
        ]
        * np.pi
...
def main():
    ...
    print(
        "working on ATL06 dir {0}, GPS file {1}".format(args.atl06, GPS_file)
    ...
    ATL06_index = os.path.join(
        os.sep, "Volumes", "ice2", "ben", "scf", HEM, args.atl06, "GeoIndex.h5"
    )
