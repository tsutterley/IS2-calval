...
def blockmedian_for_qsub(Qdata, delta):
    ...
    Re = WGS84a**2 / np.sqrt(
        (WGS84a * np.cos(d2r * lat0)) ** 2 + (WGS84b * np.sin(d2r * lat0)) ** 2
    )
    delta_lon = np.mod(Qdata.longitude - lon0 + 180.0, 360.0) - 180
    # project the Qfit latitude and longitude into northing and easting
    EN = (
        Re
        * np.c_[delta_lon * np.cos(d2r * lat0), (Qdata.latitude - lat0)]
        * np.pi
        / 180.0
    )
    Qz = Qdata.elevation.astype(np.float64)
    xm, ym, zm, ind = pt_blockmedian(
        EN[:, 0], EN[:, 1], Qz, delta, return_index=True
    )  # , randomize_evens=True)
...
def compare_seg_with_qfit(D6i, Qdata, out_template):
    ...
    EN = (
        Re
        * np.c_[
            (np.mod(Qdata.longitude - lon0 + 180.0, 360.0) - 180)
            * np.cos(d2r * lat0),
            (Qdata.latitude - lat0),
        ]
        * np.pi
...
def main():
    ...
    parser.add_argument(
        ...
        help="ICESat-2 ATL06 directory",
    )
    parser.add_argument(
        "--atl06", "-I", type=str, help="ICESat-2 ATL06 subdirectory to run"
    )
    parser.add_argument(
        "--release",
        "-R",
        type=str,
        help="ICESat-2 ATL06 data release subdirectory to run",
    )
    parser.add_argument(
        "--atm",
        "-A",
        type=lambda p: os.path.abspath(os.path.expanduser(p)),
        help="ATM directory to run",
    )
    parser.add_argument(
        "--hemisphere",
        "-H",
    ...
    parser.add_argument(
        "--query", "-Q", type=float, default=100, help="KD-Tree query radius"
    )
    parser.add_argument(
        "--median",
        "-M",
    ...
    print(
        "working on ATL06 dir {0}, ATM directory {1}".format(
            args.atl06, ATM_dir
        )
    ...
    Qfit_files = [
        os.path.join(ATM_dir, f)
        for f in os.listdir(ATM_dir)
    ...
    out_file = "vs_{0}.h5".format(args.atl06.replace(".", "_"))
    if os.path.isfile(os.path.join(out_dir, out_file)):
        print(
            "found: {0}".format(os.path.join(out_dir, out_file))
        ) if args.verbose else None

    # ATL06_index=os.path.join(os.sep,'Volumes','ice2','ben','scf',HEM,args.atl06,'GeoIndex.h5')
    ATL06_index = os.path.join(
        args.directory, args.release, args.atl06, "GeoIndex.h5"
    )
    ATL06_field_dict = {
        None: [
            "delta_time",
    ...
    D6_GI = pc.geoIndex(SRS_proj4=SRS_proj4).from_file(
        ATL06_index, read_file=True
    )
    ...
    for bin_name in sorted(D6_GI.keys()):
        ...
        D6list = D6_GI.query_xy(
            [[bin_xy[0]], [bin_xy[1]]], get_data=True, fields=ATL06_field_dict
        )
        ...
        for i_AT in D6ind:
            ...
            Qdata.index(
                np.isfinite(Qdata.elevation)
                & np.isfinite(Qdata.latitude)
